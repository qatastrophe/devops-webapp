...
- Проверить с помощью автотестов:
    - Локальный запуск:
    Для локального запуска требуется Python > 3.9

        1. Поднять сервис: `docker compose up --build -d`

        2. Выполнить: `python tests/test_service.py`
